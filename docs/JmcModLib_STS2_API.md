...
下面这张图描述的是一个子 MOD 接入 JmcModLib 后，从 STS2 调用入口、JML 完成注册和扫描，到配置 UI、热键、日志与存储实际生效的整体流程。

```mermaid
flowchart TD
    A["STS2 加载 MOD DLL"] --> B["调用子 MOD 的 ModInitializer"]
    B --> C["子 MOD 调用 ModRegistry.Register(...)"]
    C --> D["RegistryBuilder 链式配置"]
    ...
    I1 --> J["JsonConfigStorage<br/>读取或创建配置文件"]
    I4 --> J
...
JML 会将配置项接入 STS2 设置界面的“模组设置”标签页，并尽量复用游戏原生 UI 风格。
...
注意：当前 `UIDropdownAttribute` 的参数命名为 `exclude`，但对 string 下拉实际被当作候选项；对 enum 下拉可用作排除项。
...
```csharp
ConfigManager.SetStorage(new JsonConfigStorage(customRoot));
```
...
- `UIDropdownAttribute` 当前构造参数名是 `exclude`，但 string 模式下实际作为候选项使用；后续可以考虑拆分更清晰的 API。
